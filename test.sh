...
## Test Case 5
echo -n "Test case 5: Sending 1 packet and expecting receiver to print it out: "
(sleep 0.5; echo "msg 0 0 For any incoming acks, first check if it is corrupted by checking the crc. If so, discard the ack. Otherwise, we check if it is intended for this sender by comparing the ids and also if it is in the valid range."; sleep 0.5; echo "exit") | ./tritontalk -d .2 -c 0.2 -r 1 -s 1 > .output.5 2> .debug_output.5
